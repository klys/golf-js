(function (NG) {
  ...
  class MultiplayerSession {
    ...
    reportAnnouncerActivity(kind, extra = {}) {
      if (this.role === 'offline' || !kind) return false;
      const player = this.players.get(this.localPlayerKey);
      const payload = this.announcerPlayerPayload(player, {
        kind: String(kind),
        power: Number.isFinite(Number(extra.power)) ? clamp(Number(extra.power), 0, 1) : undefined,
        netTime: this.netTime,
        source: 'player-activity',
      });
      if (this.role === 'host') {
        this.emit('announceractivity', payload);
        return true;
      }
      return !!this.transport?.sendReliable(this.hostClientId, {
        type: 'announcer:activity',
        kind: payload.kind,
        power: payload.power,
      });
    }
    ...
    handlePeerMessage({ peerId, message }) {
      if (this.role === 'host') {
        ...
        } else if (message.type === 'announcer:activity' && player) {
          const kind = String(message.kind || '');
          if (kind === 'aim-start' || kind === 'aim-end') {
            this.emit('announceractivity', this.announcerPlayerPayload(player, {
              kind,
              power: Number.isFinite(Number(message.power)) ? clamp(Number(message.power), 0, 1) : 0,
              netTime: this.netTime,
              source: 'host-activity-authority',
            }));
          }
    ...
    applyShot(player, velocity) {
      ...
      player.turnsUsed += 1;
      player.shotInProgress = true;
      this.emit('announceractivity', this.announcerPlayerPayload(player, { kind: 'aim-end', netTime: this.netTime, source: 'shot-accepted' }));
